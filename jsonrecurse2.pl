...
# my ($json, $json_text, $json_dict) = (new JSON);

# file is decoded in utf8
# $json_text = slurp("amk-simple.json");
# $json_dict = $json->utf8(0)->decode($json_text);
# print $json->pretty->encode($json_dict->{amk});
# print Dumper list_api($json_dict->{amk}{group});

my $json_text = json2Dict(get_json());
my $json_dict = eval($json_text);
